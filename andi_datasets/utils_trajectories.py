...
# %% auto 0
__all__ = ['rng', 'pert', 'gaussian', 'sample_sphere', 'bm1D', 'regularize', 'normalize', 'normalize_fGN', 'trigo',
           'find_nan_segments', 'segs_inside_fov', 'inside_fov_dataset', 'motion_blur', 'plot_trajs']
...
def plot_trajs(trajs, L , N, 
    ...
    rows = 2
    if plot_labels: rows += 1
    fig, axs = plt.subplots(rows, num_to_plot, figsize = (num_to_plot*3, rows*3),
                            sharey='row', tight_layout = True)

    particle_idx = rng.choice(N,size=num_to_plot, replace=False)

    for ax, part in zip(axs.transpose(), particle_idx):
        ...
        if comp_center is not None:
            circles = [Circle((c[0],c[1]),radius=r_cercle) for c in comp_center]
            circles_collection = PatchCollection(circles, facecolor='None',
                                                 edgecolor='C1', zorder=10)
            ax[0].add_collection(circles_collection)
